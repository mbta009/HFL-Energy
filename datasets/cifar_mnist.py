...
def new_split(dataset, args, kwargs, is_shuffle = True):
    """
    split the dataset to users
    Return:
        dict of the data_loaders
    """
    client_data_configs = ConfigLoader.load_data_config(num_clients=args.num_clients)
    ...
    for i in range(len(dataset)):
        ...
        if target not in label_to_idx:
            label_to_idx[target] = []
        label_to_idx[target].append(i)
    lables = list(label_to_idx.keys())
    print(lables)
    for i in range(args.num_clients):
        data_config = client_data_configs[i]
        dict_users[i] = []
        for label in lables:
            n_label_samples = int(data_config.n_samples*data_config.dist[label])
            data = np.random.choice(label_to_idx[label], n_label_samples, replace=True)
            dict_users[i].extend(data)
...
def split_data(dataset, args, kwargs, is_shuffle = True):
    ...
    if args.iid == 1:
        ...
        data_loaders = new_split(dataset, args, kwargs, is_shuffle)
    else :
